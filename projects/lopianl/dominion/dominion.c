...
int trashCard(struct gameState* state, int player, int idx, int trash, int flag) {
  if(state->hand[player][idx] == trash) {
    discardCard(idx, player, state, flag);
    return 1;
  }
...
int playAmbassador(int choice1, int choice2, int handPos, struct gameState* state, int currentPlayer) {
  ...
  //discard played card from hand
    ...
    for (j = 0; j < choice2; j++) {
      for (i = 0; i < state->handCount[currentPlayer]; i++) {
        if(trashCard(state, currentPlayer, i, state->hand[currentPlayer][choice1], 1) == 0) {
          break;
        }
      }
  }
...
int playMine(struct gameState* state, int currentPlayer, int choice1, int choice2, int handPos) {
  ...
  gainCard(choice2, state, 2, currentPlayer);
    ...
    for (i = 0; i < state->handCount[currentPlayer]-1; i++) {
      if(trashCard(state, currentPlayer, i, j, 0) == 1) {
        break;
      }
  }
...
int cardEffect(int card, int choice1, int choice2, int choice3, struct gameState *state, int handPos, int *bonus)
{
  ...
  switch( card ) 
    ...
    case ambassador:
      return playAmbassador(choice1, choice2, handPos, state, currentPlayer);
