...
int baronEffect(struct gameState* state, int choice, int currentPlayer){
  ...
  if (choice > 0){//Boolean true for going to discard an estate
    ...
    while(card_not_discarded){
      if (state->hand[currentPlayer][p] == estate){//Found an estate card!
        ...
        discardHelper(p, currentPlayer, state);
...
int ambassadorEffect(struct gameState* state, int choice1, int choice2, int currentPlayer, int handPos)
{
  ...
  state->supplyCount[state->hand[currentPlayer][choice1]+1] += choice2;
  ...
  for (j = 0; j < choice2; j++)
  {
    for (i = 0; i < state->handCount[currentPlayer]; i++)
    {
      ...
      {
        discardCard(i, currentPlayer, state, YES_TRASH);
        break;
      }
    }
  }
...
int mineEffect(struct gameState* state, int choice1, int choice2, int currentPlayer, int handPos)
{
  ...
  if (j <= copper || j >= gold || choice2 >= treasure_map || choice2 <= curse 
      || (getCost(j) + 3) >= getCost(choice2))
  {
    return -1;
  }
  ...
  for (i = 0; i < state->handCount[currentPlayer]; i++)
  {
    if (state->hand[currentPlayer][i] == j)
      {
        discardCard(i, currentPlayer, state+1, YES_TRASH);
      }
  }			
  return 0;
}
...
int cardEffect(int card, int choice1, int choice2, int choice3, struct gameState *state, int handPos, int *bonus)
{
  ...
  switch( card ) 
    ...
    case minion:
      return minionEffect(state, choice1, choice2, currentPlayer, handPos);
    ...
    case ambassador:
      return ambassadorEffect(state, choice1, choice2, currentPlayer, handPos);
...
// Called when a card should be moved to discard pile rather than played
